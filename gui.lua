...
function GUI:Init()
    ...
    do
		...
		do
            local b = CreateFrame("CheckButton", nil, bf, "UICheckButtonTemplate")
            b:SetPoint("TOPLEFT", bf, 85, -230)
    
            b.text = b:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            b.text:SetPoint("LEFT", b, "RIGHT", 0, 1)
            b.text:SetText(L["UseValidPriceAlert"])

            b:SetScript("OnClick", function() 
                Database:SetConfig("bfusevalidalert", b:GetChecked())
            end)
            b:SetChecked(Database:GetConfigOrDefault("bfusevalidalert", true))            
		...
		do
            local b = CreateFrame("CheckButton", nil, bf, "UICheckButtonTemplate")
            b:SetPoint("TOPLEFT", bf, 175, -230)
    
            b.text = b:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            b.text:SetPoint("LEFT", b, "RIGHT", 0, 1)
            b.text:SetText(L["UseInvalidPriceAlert"])

            b:SetScript("OnClick", function() 
                Database:SetConfig("bfuseinvalidalert", b:GetChecked())
            end)
            b:SetChecked(Database:GetConfigOrDefault("bfuseinvalidalert", true))

            bf.useInvalidAlert = b
        end
        ...
        do
            ...
            local currentitem = function()
				...
				local itemCount = entry["detail"]["count"] or 1
				if(itemCount > 1) then
					item = item .. " (" .. itemCount .. ")"
				end
                return item
            ...
            local evt = function(text, playerName)
                ...
                if ask >= bid then
                    ...
                    -- L["Bid price"]
					--print("11111111")
					--local msg = playerName .. " µÄ " .. item .. " " .. GetMoneyStringL(ctx.currentprice) .. "ÓÐÐ§"
					--print(msg)
                    --SendRaidMessage(msg)
					if bf.useValidAlert:GetChecked() then
						SendRaidMessage(item .. " " .. playerName .. " " .. GetMoneyStringL(ctx.currentprice) .. L["Bid price valid"])
					end
                    --SendRaidMessage(L["Bid accept"] .. "aaa" .. " " .. item .. " " .. L["Current price"] .. " >>" .. GetMoneyStringL(ctx.currentprice) .. "<< ".. (ctx.pause and "" or L["Time left"] .. " " .. (SECOND_ONELETTER_ABBR:format(ctx.countdown))))
                else
					if bf.useInvalidAlert:GetChecked() then
						SendRaidMessage(playerName .. " "  .. L["Bid denied"] .. " " .. item .. " " .. L["Must bid higher than"] .. " " .. GetMoneyStringL(bid * 10000))
                    end
            ...
            do
                ...
                b:SetScript("OnClick", function() 
                    ...
                    SendRaidMessage(item .. " " .. L["Starting price"] .. " " .. GetMoneyStringL(ctx.currentprice) .. " ")
                    ...
                    ctx.timer = C_Timer.NewTicker(1, function()
                        ...
                        if sendalert then
							local _price = GetMoneyStringL(ctx.currentprice)
							if not ctx.currentwinner then
								_price = _price .. " " .. L["is bought in"]
							end
                            SendRaidMessage(item .. " " .. _price .. " " .. L["Time left"] .. " " .. (SECOND_ONELETTER_ABBR:format(ctx.countdown)))
                        end
                    end)
                end)
